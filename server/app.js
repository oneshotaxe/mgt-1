...
const conn = mongoose.createConnection(
  process.env.MONGODB_URL ?? 'mongodb://localhost/mgt',
  {
...
app.post('/magazine', async (req, res) => {
  ...
  let buses = await Bus.find()
    .populate({ path: 'way', populate: 'route' })
    .populate('drivers');
  ...
  for (const bus of buses) {
    for (const driver of bus.drivers) {
      ...
      try {
        const workStatuses = driver.statuses.filter((s) =>
          ['Р', '1', '2'].includes(s.value)
        );
